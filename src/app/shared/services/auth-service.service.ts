...
export class AuthServiceService {
  ...
  login(credentials: LoginRequestInterface): Observable<LoginInitialResponse> {
    return this.http
      .post<LoginInitialResponse>(`${this.apiURL}/api/auth/login`, credentials)
      .pipe(
        tap((response) => {
  ...
  checkCode(
    codeCheck: CodeCheckRequestInterface,
  ): Observable<LoginResponseInterface> {
    return this.http
      .post<LoginResponseInterface>(
        `${this.apiURL}/api/auth/login/check`,
        codeCheck,
      ...
      .pipe(
        tap((response) => {
          ...
          this.userService.getConnectedUserFromApi().subscribe({
            error: (err) =>
              console.error(
                'Échec de la récupération des détails utilisateur après connexion :',
                err,
              ),
          });
        }),
